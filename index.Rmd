...
doPanderWC <- function(winners, complete = FALSE) {
    ...
    row.names(q) <- NULL
    if (complete) {
        emphasize.italics.rows(1:7)
        lNames <- sapply(c(q[1,2], q[1,3], q[2,2], q[2,3], q[3,2], q[3,3],
                           q[4,2], q[4,3], q[5,2], q[5,3],
                           q[6,2], q[6,3], q[7,2], q[7,3]), pullLast)
        names(lNames) <- NULL
        myCaption <- sprintf("%s/%s, %s/%s, %s/%s, %s/%s, %s/%s, %s/%s, and %s/%s advance!", 
...
# pOffs <- rbind(pOffs, pooler[!(pooler$Team %in% winners), ][1:7,])
# pOffs[,2] <- 1:12
# names(pOffs)[2] <- "Seed"

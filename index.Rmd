...
doPanderWC <- function(winners, complete = FALSE, eliminate = "X") {
    ...
    if (complete) {
        ...
        lNames <- sapply(c(q[1,2], q[1,3], q[2,2], q[2,3], q[3,2], q[3,3],
                           q[4,2], q[4,3]), pullLast)
        names(lNames) <- NULL
        myCaption <- sprintf("%s/%s, %s/%s, %s/%s, and %s/%s advance!", 
...
winners <- sapply(c("B","C","D","E"), findWinners)

pOffs <- pooler[pooler$Team %in% winners,]
pOffs <- rbind(pOffs, pooler[!(pooler$Team %in% winners), ][1:4,])
pOffs[,2] <- 1:8
names(pOffs)[2] <- "Seed"
pOffs$Method <- "Wild Card"
pOffs$Method[1:4] <- "Won Group"
...
*  `r seeds[1]` vs `r seeds[8]`  
   
*  `r seeds[2]` vs `r seeds[7]`  
   
    
*  `r seeds[3]` vs `r seeds[6]` 
   
    
*  `r seeds[4]` vs `r seeds[5]`  
